...
interface FormData {
  ...
  consent_whatsapp: boolean; // Required by Meta for WhatsApp OTP
...
const AppointmentBookingForm = () => {
  ...
  // State for showing social login options
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    clinic_location: '',
    consent_pdpa: false,
    consent_whatsapp: false, // Required by Meta for WhatsApp OTP
  ...
  useEffect(() => {
    const checkLoggedInUser = async () => {
      ...
      if (user) {
        // User is logged in, hide quick fill buttons
        ...
        // Auto-fill form with user data (don't override URL params)
        setFormData(prev => ({
          ...prev,
          patient_name: prev.patient_name || profile?.full_name || user.user_metadata?.full_name || '',
          email: prev.email || user.email || '',
          whatsapp: prev.whatsapp || profile?.whatsapp_number || '',
          create_account: false, // Already has account
        }));
        
        toast.success('Welcome back! Your details have been filled in.');
      }
    };
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    // Note: OAuth will redirect and reload the page on success
  };
  ...
  useEffect(() => {
    ...
    const baseRequiredFields = [
      'patient_name', 'email', 'treatment_type',
      'preferred_date', 'time_slot', 'consent_pdpa', 'communication_preference'
    ];
    
    // Conditional fields based on communication preference
    const conditionalFields = [];
    if (formData.communication_preference === 'both') {
      conditionalFields.push('whatsapp', 'consent_whatsapp'); // WhatsApp consent required by Meta
    }
    
    // Add either preferred_clinic OR clinic_location (mutually exclusive)
    if (formData.preferred_clinic || formData.clinic_location) {
      conditionalFields.push(formData.preferred_clinic ? 'preferred_clinic' : 'clinic_location');
    } else {
      conditionalFields.push('clinic_location');
    }
    
    const fieldsToCheck = [...baseRequiredFields, ...conditionalFields];
    
    let completedFields = 0;
    fieldsToCheck.forEach(field => {
      if (field === 'preferred_date') {
        if (formData.preferred_date) completedFields++;
      } else if (field === 'consent_pdpa' || field === 'consent_whatsapp') {
        if (formData[field as keyof FormData]) completedFields++;
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.consent_pdpa) {
      newErrors.consent_pdpa = 'PDPA consent is required to proceed';
    }

    // Validate WhatsApp fields if user chose 'both'
    if (formData.communication_preference === 'both') {
      ...
      if (!formData.consent_whatsapp) {
        newErrors.consent_whatsapp = 'WhatsApp consent is required by Meta for OTP verification';
      }
    }
      ...
      <div className="max-w-2xl mx-auto">
        <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
          ...
          <CardContent className="space-y-6">
            {/* Quick Fill with Social Login - Only show if not logged in */}
            {showQuickFill && (
              <>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm font-medium text-blue-900 mb-3 text-center">
                    Already a member? Quick fill your details:
                  </p>
                ...
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="bg-white px-3 text-gray-500">Or book as guest</span>
                  </div>
                </div>
              </>
            )}
            ...
            <form id="appointment-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-start space-x-3">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleInputChange('consent_pdpa', checked)}
                    className="mt-1"
                  />
                  <div className="grid gap-1.5 leading-none flex-1">
                    <Label
                      htmlFor="consent-pdpa"
                      className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                    >
                      PDPA & Privacy Consent *
                    </Label>
                    <p className="text-xs text-gray-600">
                      I consent to the collection, use, and disclosure of my personal data for appointment booking and treatment coordination.{' '}
              ...
              {/* WhatsApp Consent Checkbox - REQUIRED BY META (Only for WhatsApp verification) */}
              {formData.communication_preference === 'both' && (
                <div className="space-y-2">
                  <div className="flex items-start space-x-3">
                    <Checkbox
                    ...
                    <div className="grid gap-1.5 leading-none flex-1">
                      ...
                      >
                        ...
                        WhatsApp Verification Consent *
                      </Label>
                      <p className="text-xs text-gray-700 font-semibold">
                        I agree to receive a 6-digit verification code and appointment updates via WhatsApp at the number provided above. Required by Meta.
                      </p>
                    </div>
                  </div>
