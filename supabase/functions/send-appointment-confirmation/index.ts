...
async function verifyTurnstileToken(token: string, ip: string): Promise<boolean> {
  ...
  // DEBUG: Check if environment variable exists (without logging the actual value)
  console.log('🔐 Turnstile Secret Status:', {
    configured: !!TURNSTILE_SECRET,
    length: TURNSTILE_SECRET ? TURNSTILE_SECRET.length : 0,
    startsWithExpected: TURNSTILE_SECRET?.startsWith('0x4') || false
  });
  
  if (!TURNSTILE_SECRET) {
    console.error('❌ CRITICAL: TURNSTILE_SECRET_KEY not configured in Supabase environment!');
    console.error('❌ Bot protection is DISABLED - blocking all requests until configured');
    return false; // STRICT: Block all requests if not configured
  }
  ...
  try {
    ...
    console.log('✅ Cloudflare siteverify response:', {
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // STRICT: Turnstile token is REQUIRED (no bypasses allowed)
    if (!bookingData.turnstile_token) {
      console.error(`❌ BLOCKED: No Turnstile token provided from IP: ${clientIP}`);
      return new Response(
        JSON.stringify({ 
    ...
    console.log('✅ Turnstile verification passed - request is legitimate');
