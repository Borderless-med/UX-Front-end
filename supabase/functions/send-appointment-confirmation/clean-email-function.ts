...
// Email service client that works without DNS changes
class EmailServiceClient {
  ...
  async sendMail(options: { from: string; to: string; subject: string; html: string }) {
    try {
      console.log("=== SENDING EMAIL VIA ORACHOPE SERVICE ===");
      console.log(`From: ${options.from}`);
      console.log(`To: ${options.to}`);
      console.log(`Subject: ${options.subject}`);
      
      // Use SendGrid API (no DNS changes required for basic sending)
      console.log("Using SendGrid API service");
      ...
      if (sendGridApiKey) {
        const sendGridResponse = await fetch("https://api.sendgrid.com/v3/mail/send", {
          ...
          body: JSON.stringify({
            ...
            from: { 
              email: "noreply@sgsmile.app", // Generic sender
              name: "SG-JB Dental"
            },
            reply_to: { 
              email: this.username, // Reply goes to contact@orachope.org
              name: "SG-JB Dental Support"
            },
            content: [{
              type: "text/html",
              value: options.html
        ...
        if (sendGridResponse.ok) {
          ...
          console.log(`Reply-To set to: ${this.username}`);
          return { success: true, message: "Email sent via SendGrid", service: "sendgrid" };
        } else {
      ...
      // Fallback: Use Mailgun API (also no DNS changes needed for basic sending)
      console.log("Trying Mailgun API as fallback");
      ...
      if (mailgunApiKey) {
        ...
        formData.append("from", `SG-JB Dental <noreply@${mailgunDomain}>`);
        formData.append("to", options.to);
        formData.append("subject", options.subject);
        formData.append("html", options.html);
        formData.append("h:Reply-To", this.username);
        ...
        if (mailgunResponse.ok) {
          ...
          return { success: true, message: "Email sent via Mailgun", service: "mailgun" };
        }
      }
      
      // Final fallback: Log email for manual processing
      console.log("All email services unavailable - logging email for manual processing");
      console.log(`Email Details - To: ${options.to}, Subject: ${options.subject}`);
      console.log(`Reply-To: ${this.username} (OraChope domain)`);
      
      // Return success so booking isn't blocked, but with a warning
      return { success: true, message: "Email queued for manual processing", service: "manual" };
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Initialize Email Service (no DNS changes required)
    ...
    console.log(`Email Service Configuration for: ${SMTP_USER}`);
